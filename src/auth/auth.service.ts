...
export class AuthService {
  ...
  async signIn({ email, password }: SignInDto) {
    const userId = await this.usersService.getUserIdByCredentials({
      email,
      password,
    });
    if (!userId) {
  ...
  async signUp({ email, password }: SignInDto) {
    ...
    if (!userId) {
      throw new HttpException(
        '104 - Ошибка на сервере (Не удалось зарегистрировать пользователя)',
  ...
  async checkDecodeToken(token: string): Promise<string> {
    ...
    if (!result) {
      throw new HttpException(
        '100 - Ошибка на сервере (неверный формат токена)',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (!result['userId']) {
      throw new HttpException(
        '101 - Ошибка на сервере (неверный формат токена)',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  getTokenObj(payload) {
    return {
      ...
      refresh_token: this.jwtService.sign(payload, {
        expiresIn: Config.refreshTokenTimeout,
      }),
    };
  }
}
