...
export class NotesService {
  ...
  async getRelativeNotes(
    req: Request,
    { pagination, search, relativeId }: IRelativesNotesQuery,
  ) {
    const userId = getReqUserId(req);
    const searchData: ISearchData = {
      search,
  ...
  async update(req: Request, { id, data }: IUpdateNote) {
    ...
    if (!result) {
      throw new HttpException(
        '114 - Запись не найдена',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (result.creator !== userId) {
      throw new HttpException(
        '116 - Нет прав на редактирование записи',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async delete(req: Request, id: string) {
    ...
    if (note.creator !== userId) {
      throw new HttpException(
        '115 - Нет прав на удаление записи',
        HttpStatus.BAD_REQUEST,
      );
    }
