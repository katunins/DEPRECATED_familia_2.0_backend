...
export class UsersService {
  ...
  async getUserIdByCredentials({email, password}: SignInDto): Promise<string> {
  ...
  async createUser({email, password}: SignInDto): Promise<User> {
    ...
    if (sameUser) {
      throw new HttpException('106 - Пользователь с таким email уже существует', HttpStatus.BAD_REQUEST);
    }
